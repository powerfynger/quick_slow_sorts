...
double work_time_q, work_time_s;
void sortt(int i){
    clock_t tic = clock();
    int k, x;
    for(int j = 1; j <= i - 1;j++){
    ...
    clock_t toc = clock();    
    work_time_s = (toc-tic);///CLOCKS_PER_SEC;
}
...
void qsortt(int l, int r){
    ...
    if(k < r) qsortt(k, r);
    clock_t toc = clock();
    work_time_q = (toc-tic);///CLOCKS_PER_SEC;
}    
//asd.txt -s -q -all
int main(int argc, char *argv[]){
    ...
    if(argv[2][0] == '-' && argv[2][1] == 'a' && argv[2][2] == 'l' && argv[2][3] == 'l'){
        ...
        printf("quick time: %lf ms\n", work_time_q);
        printf("slow time: %lf ms\n", work_time_s);
        return 0;
    }   
    printf("Неизвестный аргумент \"%s\", используйте:\n-s - медленная сортировка\n-q - быстрая сортировка\n-all - все виды сортировок\n", argv[2]);
    return 0;
